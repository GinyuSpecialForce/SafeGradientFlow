...
A Python program that implements safe gradient flow with hard constraint enforcement using Adam adaptive learning rate, 3D visualization, and multi-start optimization.
...
- Automatically suggesting learning rate and alpha values
...
- Supporting multi-start optimization with noise
...
**2. Noise**: Adds random "jumps" to the learning rate to escape local minima.
...
# Multi-start with noise
python3 safe_gradient.py -f "sin(x1)*cos(x2) + 0.1*(x1**2 + x2**2)" -g "sin(x1) + cos(x2) => 0.5" --multi 10 --range -2,2 --noise 0.3 --noise_freq 8
```
...
```bash
# Find the global minimum of a complex function with 10 starts
python3 safe_gradient.py -f "20 + x1**2 + x2**2 - 10*(cos(2*pi*x1) + cos(2*pi*x2))" -g "x1**2 + x2**2 => 1" --multi 10 --range -3,3 --noise 0.3 --noise_freq 8 --save my_run
```
